...
/*
 * To normalize Twitter dataset produced by Alex or emails by hashing features first. 
 * Input: [ Docid <feature,freq>+ ]+ whether feature is hashed or not.  
 */
public class FrequenciesNormalizer {
	public static void main(String args[]) {
		try {
			if (args.length != 0) {
				System.out
						.println("Usage: <inputfile of vectors> <outputfile> will normalize frequencies.");
				System.exit(2);
			}
			DataInputStream in = new DataInputStream(new FileInputStream( // args[0]));
					"/Users/Hadoop/data/tweets/2"));
			BufferedWriter out = new BufferedWriter(new FileWriter(// args[1]));
					"/Users/Hadoop/cpc/cpc-apss/data/twitter/1m-normalized"));
